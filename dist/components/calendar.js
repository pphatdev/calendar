...
export class Calendar {
    constructor(element, settings) {
        ...
        this.init = () => {
            ...
            if (!element.classList.contains('calendar-initialized')) {
                ...
                if (windowWidth <= this.$breakpoints.tablet) {
                    ...
                    if (!this.options.sidebarDisplayDefault)
                        element.classList.add('sidebar-hide');
                    /*  set event-hide visibility on load */
                    if (!this.options.eventDisplayDefault)
        ...
        // Format date
        this.formatDate = (date, format, language) => {
            if (!date)
                return;
        ...
        this.isBetweenDates = (active_date, dates) => {
            var sd, ed;
        ...
        this.hasSameDayEventType = (date, type) => {
            let eventLength = 0;
        ...
        this.setTheme = (themeName) => {
            ...
            if (this.options.theme)
                $(this.$elements.calendarEl).removeClass(prevTheme);
            if (this.options.theme !== 'default')
                $(this.$elements.calendarEl).addClass(this.options.theme);
        };
        ...
        this.buildTheBones = () => {
            ...
            if (!this.$elements.calendarEl.html()) {
                ...
                let monthlist = '';
                let datelist = '';
                this.$label.days.forEach(label => {
                    datelist +=
                        `<td class="${(label === this.initials.weekends.sat || label === this.initials.weekends.sun)
                            ? 'calendar-header-day --weekend'
                            : 'calendar-header-day'}">${label}
                    </td>`;
                });
                this.$label.months.forEach((label, i) => {
                    monthlist += `<li class="month" role="button" data-month-val="${i}"> ${this.initials.dates[this.options.language].months[i]}</li>`;
                });
                markup = `
            ...
            `;
                ...
                if (!this.$elements.sidebarEl)
                    this.$elements.sidebarEl = $(this.$elements.calendarEl).find('.calendar-sidebar');
                if (!this.$elements.innerEl)
                    this.$elements.innerEl = $(this.$elements.calendarEl).find('.calendar-inner');
                if (!this.$elements.eventEl)
                    this.$elements.eventEl = $(this.$elements.calendarEl).find('.calendar-events');
                if (this.options.sidebarToggler) {
                ...
                `);
                    if (!this.$elements.sidebarToggler)
                        this.$elements.sidebarToggler =
        ...
        this.buildEventList = () => {
            ...
            this.$active.events = [];
            // Event date
            const title = this.formatDate(this.$active.date, this.options.eventHeaderFormat, this.options.language);
            // Event list
        ...
        this.removeEventList = (eventData) => {
            let markup;
            let eventListEl = this.$elements.eventEl.find('.event-list');
            if (eventListEl.find('[data-event-index="' + eventData + '"]').length === 0)
        ...
        this.buildSidebarMonths = () => {
            return (this.$elements.sidebarEl.find('.calendar-months > [data-month-val]').removeClass('active-month'),
        ...
        this.buildCalendar = () => {
            this.calculateDays();
            const title = this.formatDate(new Date(this.$label.months[this.$active.month] + ' 1 ' + this.$active.year), this.options.titleFormat, this.options.language);
            ...
            for (var i = 0; i < 9; i++) {
                ...
                this.$label.days.forEach((day, j) => {
                    ...
                    if (date <= this.monthLength && (i > 0 || j >= this.startingDay)) {
                        grid += `<td class="${(day === this.initials.weekends.sat || day === this.initials.weekends.sun)
                            ? 'calendar-day --weekend'
                            : 'calendar-day'}">
                            <div class="day" role="button" data-date-val="${this.formatDate(this.$label.months[this.$active.month] + ' ' + date + ' ' + this.$active.year, this.options.format)}">${date}</div>`;
                        date++;
                    }
                    else {
                        grid += '<td>';
        ...
        ****************/
         ...
         * @returns
         *  */
        this.selectEvent = (event, callback) => {
        ...
        // Select month
        this.selectMonth = function (event) {
            var _THIS = this;
        ...
        this.selectDate = (event) => {
            ...
            if (typeof event === 'string' || typeof event === 'number' || event instanceof Date) {
                ...
                if (this.$active.year !== year)
                    this.selectYear(year);
                if (this.$active.month !== month)
        ...
        this.toggleOutside = (event) => {
            const hasSidebar = !this.$elements.calendarEl.hasClass('sidebar-hide');
            const hasEvent = !this.$elements.calendarEl.hasClass('event-hide');
            const isInnerClicked = event.target === this.$elements.innerEl[0];
            if (hasSidebar && isInnerClicked)
                this.toggleSidebar(false);
            if (hasEvent && isInnerClicked)
                this.toggleEventList(false);
        };
        // Toggle Sidebar
        this.toggleSidebar = (event) => {
            ...
            if (windowWidth <= this.$breakpoints.tablet && windowWidth > this.$breakpoints.mobile) {
                const hasSidebar = !this.$elements.calendarEl.hasClass('sidebar-hide');
                const hasEvent = !this.$elements.calendarEl.hasClass('event-hide');
                if (hasSidebar && hasEvent)
                    this.toggleEventList();
            }
        };
        // Toggle Event list
        this.toggleEventList = (event) => {
            (event === undefined || event.originalEvent)
            ...
            if (windowWidth <= this.$breakpoints.tablet && windowWidth > this.$breakpoints.mobile) {
                const hasEvent = !this.$elements.calendarEl.hasClass('event-hide');
                const hasSidebar = !this.$elements.calendarEl.hasClass('sidebar-hide');
                if (hasEvent && hasSidebar)
                    this.toggleSidebar();
            }
        };
        //  Add Calendar Event(s)
        this.addCalendarEvent = (arr) => {
            const addEvent = (data) => {
                if (!data.id) {
                ...
                if (this.$active.event_date === data.date)
                    this.addEventList(data);
